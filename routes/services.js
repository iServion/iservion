...
var MYMODEL = require('./../models/services');
var zSSH = require("./../components/zSSH2");
        ...
        fields.forEach(function (item) {
            if (item == "no") {
                ...
                zRoute.actionButtons(levels, row, MYMODEL.table, function (cb) {
                    var view = cb.view;
                    //view += `<button class="btn btn-success">Success</button>`
                    arr.push(view);
                });
            } else {
                if (item == 'status') {
                    arr.push(row.status == 1 ? `<i class="fas fa-power-off btn-success boxy btn-rounded"></i>` : `<i class="fas fa-power-off btn-danger boxy btn-rounded"></i>`);
                } else {
...
router.post('/create', csrfProtection, async(req, res) => {
    ...
    try {
        ...
        if (data.status == 0) {
            await zSSH.exec(result.id, `systemctl stop  ${data.title}`, function (data) {
            });
            await zSSH.exec(result.id, `systemctl status  ${data.title}`, function (data) {
                io.to(res.locals.token).emit("message", data.data)
            });
        } else {
            await zSSH.exec(result.id, `systemctl start  ${data.title}`, function (data) {
            });
            await zSSH.exec(result.id, `systemctl status  ${data.title}`, function (data) {
                io.to(res.locals.token).emit("message", data.data)
            });
...
router.post('/update/:id', csrfProtection, async(req, res) => {
    ...
    try {
        let id = req.params.id;
        var dataOld = await connection.result({
            table: MYMODEL.table,
            where: {
                id: id
            }
        ...
        if (dataOld.status != data.status) {
            var status = ["Unactive", "Active"];
            await connection.insert({
                ...
                data: {
                    ...
                    status_label: status[result.status],
                    title: `Change status to ${status[result.status]}`,
                    description: `Service ${result.title} has changed to ${status[result.status]} by ${res.locals.zuser.fullname}`,
        ...
        if (data.status == 0) {
            await zSSH.exec(data.server_id, `systemctl stop  ${data.title}`, function (data) {
            });
            await zSSH.exec(data.server_id, `systemctl status  ${data.title}`, function (data) {
                io.to(res.locals.token).emit("message", data.data)
            });
        } else {
            await zSSH.exec(data.server_id, `systemctl start  ${data.title}`, function (data) {
            });
            await zSSH.exec(data.server_id, `systemctl status  ${data.title}`, function (data) {
                io.to(res.locals.token).emit("message", data.data)
            });
