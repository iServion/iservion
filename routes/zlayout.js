...
var MYMODEL = require('./../models/zlayout');
...
<% if(renderBody){ include("./../"+renderBody) } %>
...
router.post('/create', csrfProtection, async (req, res) => {
    ...
    try {
        ...
        data.name = Util.toName(data.name);
        var layoutCheck = await zPage.layoutCheck(data.name);
        if(layoutCheck) {
            return res.json(Util.flashError("name is used"));
...
router.delete('/delete', async (req, res) => {
    ...
    try {
        ...
        await zPage.layoutDelete(result.name)
