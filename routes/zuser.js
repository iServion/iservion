...
router.post('/create', csrfProtection, async (req, res) => {
    ...
    try {
        ...
        data.token = Util.generateUnique();
        var validator = zRoute.validator(data, MYMODEL);
        if(validator.status == 0) return res.json(validator.message);

        if(data.password.length < 6) {
            return res.json(Util.flashError("Password must be 6 length"));
        }
        data.password = Util.hash(data.password);
        data.email = data.username;
        data.token = Util.uuid();
        data.language = "lang_en";
        var result = await zRoute.insertSQL(req, res, MYMODEL.table, data);
        var userCompany = {}
        userCompany.company_id = res.locals.companyId;
        userCompany.role_id= data.role_id;
        userCompany.user_id = result.id;
        await connection.insert({table:"zuser_company", data:userCompany});
    } catch (err) {
...
router.delete('/delete', async (req, res) => {
    ...
    if(id == 1){
        return res.json(Util.flashError("Owner can not be deleted!!"));
        return false;
    }
    try {
        ...
        await zRoute.deleteSQL("zuser_company", result.id, res.locals.companyId);
...
router.get('/approval/:id', async(req, res) => {
    ...
    let zForms = {};
    try {
        let result = await connection.result({table: MYMODEL.table, where: {id: id}});
        if (!result.id) {
            req.session.sessionFlash = Util.flashError(LANGUAGE['data_not_found']);
            return res.redirect('/' + MYMODEL.routeName);
        }
        moduleLib.editor(req, res);
        moduleLib.addScript(req, res, ``);
        data = await zRoute.viewTable(req, res, MYMODEL, result);
        var contentScript = `$("#submit_draft").on("click", function () {
            $("#status").val(1);
...
`;
        ...
        var MODEL_APROVALS = require("./../models/zapprovals")
        var rows = await connection.results({
            table: "zapprovals",
        ...
        if(rows.length) {
            ...
            obj.template = zfields.approval_content;
            var zapprovals = zfields.zapprovals ? JSON.parse(zfields.zapprovals) : {};
            var title = zapprovals.title;
            if(title) {
                var titleData = title;
                for(var key in data) {
            ...
            if(zapprovals.hasOwnProperty("approvers")) {
                obj.approvers = zapprovals.approvers;
                obj.knowings = zapprovals.knowings;
                obj.type = 1;
            }
            await connection.insert({table:"zapprovals", data:obj});
            data2 = obj;
        }
        let relations = await zRoute.relations(req, res, "zapprovals");
        zForms = zRoute.formField(req, res, MODEL_APROVALS, relations, data2);
        await zRoute.moduleLib(req, res, MODEL_APROVALS, relations, zForms);
    } catch (err) {
        debug(req, res, err);
        res.send("Err");
    }
...
router.get('/preview/:token', async(req, res) => {
    ...
    try {
        ...
        var MYMODEL2 = require("./../models/zapprovals")
        data = await zRoute.viewTable(req, res, MYMODEL2, results[0], true);

        var row = await connection.result({table: results[0].table, where: {id: results[0].id_data}});
...
router.get('/excel-query', async (req,res) => {
    await zRoute.attributeData(res, MYMODEL);
    let options = MYMODEL.options;
    let fields = res.locals.visibles.filter(item => item != "no" && item != "actionColumn");
    let query = req.query;
    query.sortField = query.sortField || "id";
    query.sortOrder = query.sortOrder || "DESC"
    let grid = connection.grid(query, options);
    let results = await connection.results({table: MYMODEL.table, statement: grid.statement, values: grid.arr});
...
router.get('/sample', async (req, res) => {
    ...
    for (let i = 0; i < MYMODEL.keysExcel.length; i++) {
        if (!Util.in_array(MYMODEL.keysExcel[i], Util.nots)) {
            if (!Util.in_array(MYMODEL.keysExcel[i], CONFIG.generator.imagesName)) {
...
router.post('/import', async(req, res) => {
    ...
    for (var prop in result[1]) {
        ...
        hd += '<td>' + result[0][prop] + '</td>';
        headers.push({i: i, prop: prop, value: result[0][prop]});
        i++;
    }
    hd += '<td> ' + LANGUAGE['noted'] + ' </td></tr></thead>';
    dataObj.msg = hd;
    ...
    for (var i = 3; i < result.length; i++) {
        ...
        for (var u = 0; u < headers.length; u++) {
            if (Object.prototype.hasOwnProperty.call(result[i], headers[u].prop)) {
                hd += '<td>' + result[i][headers[u].prop] + '</td>';
            } else {
                hd += '<td> </td>';
            }
        }
        try {
            ...
            hd += '<td><span class="badge badge-success">' + LANGUAGE['success'] + '</span></td>';
        } catch (err) {
            hd += '<td><span class="badge badge-danger">' + err + '</span></td>';
        }
        hd += '</tr>';
        dataObj.msg = hd;
        io.to(room).emit('import', hd);
