...
var MYMODEL = require('./../models/zcompany');
...
router.post('/create', csrfProtection, async (req, res) => {
    ...
    try {
        ...
        var zmenu = await connection.result({
            table : "zmenu",
            where : {
                active : 1
            }
        ...
        var menuData = {
            ...
            name : zmenu.name,
            json : JSON.stringify(zmenu.json),
            active : 1
        }
        await connection.insert({
            table: "zmenu",
        ...
        await zRoute.handleSession(req,res.locals.zuser);
...
router.get('/update/:id', csrfProtection, async (req, res) => {
    ...
    var results = await connection.results({table: MYMODEL.table, where: {id: id}});
    if (results.length == 0) {
...
router.delete('/delete', async (req, res) => {
    ...
    try {
         ...
         if(id == 1) {
            return res.json(Util.flashError("Mandatory company can not be deleted!"));
        }
