...
var MYMODEL = require('./../models/zapi');



router.get('/', async(req,res) => {
    var myapi = await connection.results({
        table : "zapi",
        where : {
            username : res.locals.zuser.username
        }
    });

    var url = CONFIG.app.url;
    var api = {}
    var hasApi = myapi.length ? true :false;
    if(hasApi) {
        api= myapi[0];
    }
    res.render(`layouts/${layout}`, {
        ...
        api : api,
        url:url,
        titleHeader: "Server API",
        //renderHead: `${MYMODEL.routeName}/indexcss.ejs`,
        renderBody: `${MYMODEL.routeName}/zapi.ejs`,
        renderEnd: `${MYMODEL.routeName}/zapijs.ejs`
    });
});
...
router.get('/excel-query', async (req,res) => {
    ...
    var fields = res.locals.visibles.filter(item => item != "no" && item != "actionColumn");
    var rows = [];
    var body = {}
    if(results.length) {
