...
var zPage = require('./../components/zPage');
    ...
    rows.forEach(function (row, index) {
        ...
        fields.forEach(function (item) {
            if (item == "no") {
                ...
                zRoute.actionButtons(levels, row, MYMODEL.table,function (datas) {
                    var views = datas.view;
                    views += `&nbsp;<i data-id="${row.id}" title="Go" class="fas fa-plane  boxy-small btn-success btn-rounded"  onclick="location.href='${row.page}'"></i>`;
                    arr.push(views)
                });
            } else {
...
router.get('/create', csrfProtection, async(req, res) => {
    ...
    //add script
    data.server_code = `var myusernameis  = "clark Kent";

    res.render(\`layouts/${layout}\`, {
        myusernameis: myusernameis
    });`
    
data.client_code = `<h1>My Username is <%- myusernameis %></h1>`;
...
router.post('/create', csrfProtection, async (req, res) => {
    ...
    try {
        ...
        //data.page = "/"+Util.toName(data.page);
