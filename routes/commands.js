...
var MYMODEL = require('./../models/commands');
        ...
        fields.forEach(function (item) {
            if (item == "no") {
                ...
                zRoute.actionButtons(levels, row, MYMODEL.table, function (cb) {
                    var view = '';
                    if(row.command ||  row.long_command) {
                        view = `<i data-id="${cb.row.id}" title="Run/Execute" class="fab fa-cuttlefish gridrun boxy-small  btn-danger btn-rounded"></i>&nbsp;`;
                    }
                    view += cb.view;
                    arr.push(view);
                });
